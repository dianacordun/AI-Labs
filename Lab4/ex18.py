...
class Graph:  # graful problemei
    ...
    """Pt ex 18 """
    # starea este scop daca toate stivele din ea au lungimi egale
    def testeaza_scop(self, nodCurent):
        lungime = len(nodCurent.info[0])
        for el in nodCurent.info:
            if len(el) != lungime:
                return False
        return True
    ...
    # euristica banala
    def calculeaza_h(self, infoNod, tip_euristica="euristica admisibila 3"):
        # euristica pt 18
        # nivelul maxim este nr de elemente impartit la nr de stive
        if tip_euristica == "euristica admisibila 3":
            euristici = []
            nr_elem = len([elem for stiva in infoNod for elem in stiva])
            nr_stive = len(infoNod)
            nivel_maxim = nr_elem / nr_stive
            h = 0

            for stiva in infoNod:
                if len(stiva) > nivel_maxim:
                    h += 1 + ord(stiva[-1]) - ord("a")
            euristici.append(h)
...
def a_star(gr, nrSolutiiCautate, tip_euristica):
    ...
    while len(c) > 0:
        nodCurent = c.pop(0)
        # Conditie! Numarul de elemente/blocuri trebuie sa fie divizibil cu numarul de stive
        nr_elem = 0
        for stiva in nodCurent.info:
            for _ in stiva:
                nr_elem += 1
        if nr_elem % len(nodCurent.info):
            print("Nu exista stare scop!")
            break
...
nrSolutiiCautate = 3
a_star(gr, nrSolutiiCautate=3, tip_euristica="euristica admisibila 3")
